package demise.client.module.modules.minigames;
...
import static demise.client.utils.Utils.Java.round;
...
public class BedwarsOverlay extends Module {
    ...
    private boolean drawError(ScaledResolution sr, FontRenderer fr) {
        ...
        if (Utils.URLS.hypixelApiKey.isEmpty()) {
            fr.drawString(noApiKey, (int) (overlayWidth + overlayX.getInput() - overlayWidth / 2 - (double) fr.getStringWidth(noApiKey) / 2), (int) textY, errorColour);
            textY += fr.FONT_HEIGHT + marginTextY.getInput();
            return true;
        } else if (!Utils.Client.othersExist()) {
            fr.drawString(noPlayers, (int) (overlayWidth + overlayX.getInput() - overlayWidth / 2 - (double) fr.getStringWidth(noPlayers) / 2), (int) textY, errorColour);
            textY += fr.FONT_HEIGHT + marginTextY.getInput();
            return true;
        }
