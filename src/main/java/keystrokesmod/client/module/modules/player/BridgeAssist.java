...
public class BridgeAssist extends Module {
    ...
    public void onRenderTick(TickEvent.RenderTickEvent e) {
        ...
        if (gliding) {
            ...
            float yaw = fuckedYaw - (fuckedYaw / 360) * 360;
            float pitch = fuckedPitch - (fuckedPitch / 360) * 360;
            ...
            if (mc.thePlayer.rotationYaw < this.waitingForYaw)
                mc.thePlayer.rotationYaw += (float) this.speedYaw;

            if (mc.thePlayer.rotationYaw > this.waitingForYaw)
                mc.thePlayer.rotationYaw -= (float) this.speedYaw;

            if (mc.thePlayer.rotationPitch > this.waitingForPitch)
                mc.thePlayer.rotationPitch -= (float) this.speedPitch;
    ...
    public void aimAt(float pitch, float yaw, float fuckedYaw, float fuckedPitch) {
        if (setLook.isToggled()) {
            mc.thePlayer.rotationPitch = pitch + (fuckedPitch / 360) * 360;
